    ...
    <Card className="shadow-sm">
      ...
      <CardContent className="space-y-4 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-3">
          ...
          <Button variant="ghost" size="sm" onClick={resetDates} className="gap-2">
              ...
              <TableBody>
                {filtered.map((row) => {
                  ...
                  const runningMinutes =
                    inAt && !outAt ? Math.max(0, Math.round((now.getTime() - inAt.getTime()) / 60000)) : null;
                  const workedLabel =
                    runningMinutes != null
                    ...
                    <TableRow key={key}>
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {row.employee?.department?.name || "—"} · {row.employee?.position?.name || "—"}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-sm text-muted-foreground">{workedLabel}</TableCell>
                      <TableCell className="text-sm text-muted-foreground">
                        {row.lateMinutes != null ? formatMinutesToTime(row.lateMinutes, false) : "—"}
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground">
                        {row.undertimeMinutes != null ? formatMinutesToTime(row.undertimeMinutes, false) : "—"}
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground">
